...
def render_key_static(
    ...
    fig = plt.figure(figsize=(9, 6), dpi=130)
    ax = fig.add_subplot(111, projection="3d")
    _setup_axes(fig, ax)

    frame = frames[-1]
    val_min, val_max = value_range
    alpha = _alpha_from_values(frame.value, val_min, val_max)
    ax.scatter(frame.x, frame.y, frame.z, s=50, c=_rgba(color, alpha), edgecolors="none")
...
def render_key_animation(
    ...
    def update(frame_idx: int) -> list[Any]:
        ...
        alpha = _alpha_from_values(current.value, val_min, val_max)
        ax.scatter(
            current.x,
            current.y,
            current.z,
...
def render_environment_animation(
    ...
    def update(frame_idx: int) -> list[Any]:
        ...
        for key, series in all_frames.items():
            ...
            alpha = _alpha_from_values(frame.value, val_min, val_max, low=0.12, high=0.88)
            ax.scatter(
                frame.x,
                frame.y,
                frame.z,
...
def build_3d_time_series_visualization(
    data: dict[str, list[Any]], amount_nodes: int = 28, dims: int = 360, output_dir: str = "output_dir"
) -> None:
    print("[engine] start")
